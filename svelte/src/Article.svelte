...
<div class="historyLine" class:toRight={change && index % 2 == 0} class:toLeft={change && index % 2 == 1} class:historyLineOpen={open}>
    <div
        class="firstDiv"
        on:click={() => {
            open = !open;
    ...
    >
        ...
        <div class="plusOrMinus iconShowMore">
            {#if open}
                <svg focusable="false" {viewBox} aria-hidden="true"><path d={svg1} /></svg>
            {:else}
                <svg focusable="false" {viewBox} aria-hidden="true"><path d={svg2} /></svg>
            {/if}
        </div>
    </div>
    <div class="secondDiv">
